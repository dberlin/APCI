...
ssize_t read_apci(struct file *filp, char __user *buf, size_t len, loff_t *off)
{
...
#endif
{
	...
	case apci_get_device_info_ioctl:
...
#else
		status = access_ok(VERIFY_WRITE, arg, sizeof(info_struct));
#endif
		...
		status = copy_to_user((info_struct *)arg, &info,
				      sizeof(info_struct));
	...
	case apci_write_ioctl:
		...
		case IO:
			apci_info("performing I/O write to %llX\n",
				  ddata->regions[io_pack.bar].start +
					  io_pack.offset);
		...
		case MEM:
			...
			case BYTE:
				apci_devel("writing byte %02X to %llX\n",
					   ...
					   ddata->regions[io_pack.bar].start +
						   io_pack.offset);
				iowrite8(io_pack.data,
					 ddata->regions[io_pack.bar]
							 .mapped_address +
						 io_pack.offset);
				break;
	...
	case apci_read_ioctl:
		...
		case IO:
			...
			case BYTE:
				io_pack.data =
					inb(ddata->regions[io_pack.bar].start +
					    io_pack.offset);
				apci_devel(
					"performed byte read from %llX, got %02X\n",
					ddata->regions[io_pack.bar].start +
						io_pack.offset,
					io_pack.data);
				break;
			...
			case WORD:
				io_pack.data =
					inw(ddata->regions[io_pack.bar].start +
					    io_pack.offset);
				apci_devel(
					"performed word read from %llX, got %04X\n",
					ddata->regions[io_pack.bar].start +
						io_pack.offset,
					io_pack.data);
				break;
			...
			case DWORD:
				io_pack.data =
					inl(ddata->regions[io_pack.bar].start +
					    io_pack.offset);
				apci_devel(
					"performed dword read from %llX, got %08X\n",
					ddata->regions[io_pack.bar].start +
						io_pack.offset,
					io_pack.data);
				break;
		...
		case MEM:
			...
			case BYTE:
				io_pack.data =
					ioread8(ddata->regions[io_pack.bar]
							.mapped_address +
						io_pack.offset);
				apci_devel(
					"performed read from %llX, got %02X\n",
					ddata->regions[io_pack.bar].start +
						io_pack.offset,
					io_pack.data);
				break;
			...
			case WORD:
				io_pack.data =
					ioread16(ddata->regions[io_pack.bar]
							 .mapped_address +
						 io_pack.offset);
				apci_devel(
					"performed read from %llX, got %04X\n",
					ddata->regions[io_pack.bar].start +
						io_pack.offset,
					io_pack.data);
				break;
			...
			case DWORD:
				io_pack.data =
					ioread32(ddata->regions[io_pack.bar]
							 .mapped_address +
						 io_pack.offset);
				apci_devel(
					"performed read from %llX, got %08X\n",
					ddata->regions[io_pack.bar].start +
						io_pack.offset,
					io_pack.data);
				break;
	...
	case apci_wait_for_irq_ioctl:
		...
		wait_event_interruptible(ddata->wait_queue,
					 ddata->waiting_for_irq == 0);

		if (ddata->irq_cancelled == 1)
			return -ECANCELED;
	...
	case apci_set_dma_transfer_size:
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 18, 0))
		status = access_ok((const void *)arg,
			...
			return -EACCES;
		{
			...
			ddata->dma_virt_addr = dma_alloc_coherent(
				&(ddata->pci_dev->dev),
				ddata->dma_num_slots * ddata->dma_slot_size,
				&(ddata->dma_addr), GFP_KERNEL);
			if (ddata->dma_virt_addr == NULL)
				return -ENOMEM;
	...
	case apci_data_ready:
			...
			return -EACCES;
		{
			...
			if ((ddata->dma_last_buffer < 0) ||
				...
				if (last_valid == -1)
					last_valid = ddata->dma_num_slots - 1;

				apci_debug(
					"last_valid = %d, ddata_dma_last_buffer = %d\n",
					last_valid, ddata->dma_last_buffer);

				if (last_valid >= data_ready.start_index) {
					data_ready.slots =
						last_valid -
						data_ready.start_index + 1;
				} else {
					data_ready.slots =
						ddata->dma_num_slots -
						data_ready.start_index +
						last_valid + 1;
				}
			}

			apci_debug(
				"data_ready.start_index = %d, ddata->dma_last_buffer = %d, data_ready.slots = %d\n",
				data_ready.start_index, ddata->dma_last_buffer,
				data_ready.slots);
			...
			apci_debug(
				"start_index = %d, first_valid = %d, num_slots = %d, discarded = %d\n",
				data_ready.start_index, ddata->dma_first_valid,
				data_ready.slots, data_ready.data_discarded);
	...
	case apci_write_buff_ioctl:
		status = copy_from_user(&buff_pack, (buff_iopack *)arg,
					sizeof(buff_iopack));
		if (status != 0) {
			apci_error("access_ok failed\n");
			return status;
		...
		switch (is_valid_addr(ddata, buff_pack.bar,
				      buff_pack.bar_offset)) {
		case IO:
			apci_devel(
				"performing buffered I/O write %d times to %llX\n",
				buff_pack.length,
				ddata->regions[buff_pack.bar].start +
					buff_pack.bar_offset);

			switch (buff_pack.size) {
			case BYTE: {
				__u8 *pdata = (__u8 *)(ddata->dac_fifo_buffer +
						       buff_pack.mmap_offset);
				for (count = 0; count < buff_pack.length;
				     count++) {
					outl(pdata[count],
					     ddata->regions[buff_pack.bar].start +
						     buff_pack.bar_offset);
		...
		case MEM:
			apci_info(
				"performing buffered MEM write %d times to %llX\n",
				buff_pack.length,
				ddata->regions[buff_pack.bar].start +
					buff_pack.bar_offset);
			switch (buff_pack.size) {
